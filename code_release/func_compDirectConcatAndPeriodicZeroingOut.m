function func_compDirectConcatAndPeriodicZeroingOut (videoFileName_arr, targetLocalSNR_dB_arr, numOfRealizations)
...
for vid_ind = 1 : length(videoFileName_arr)
    ...
    for snr_ind = 1 : length(targetLocalSNR_dB_arr)
        ...
        for realization_ind = 1 : numOfRealizations
            ...
            % make their lenghts the same assuming that power sig was
            % captured simutaneously with video recording
...
width1 = .3; %// small width to avoid overlap
width2 = .9; %// small width to avoid overlap
